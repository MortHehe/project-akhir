...
@section('content')
    ...
    @php
        ...
        $thisMonthSpent = $user->clientOrders()
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <div class="value">${{ number_format($thisMonthSpent, 0) }}</div>
        ...
        <div class="stat-card">
            ...
            <div class="subtitle">All time spending</div>
        </div>
    </div>
    ...
    @php
        $orders = $user->clientOrders()->with('freelancer')->latest()->get();
    @endphp
    ...
    <div class="orders-grid">
            ...
            <div class="order-card">
                <div class="order-header">
                    <div>
                        ...
                        <div class="client-name">Freelancer: {{ $order->freelancer ? $order->freelancer->name : 'Not assigned' }}</div>
                    </div>
                ...
                @if($order->deadline && in_array($order->status, ['accepted', 'in_progress']))
                    @php
                ...
                <div class="order-meta">
                    <div class="meta-item">
                        ...
                        <span class="meta-value">${{ number_format($order->price, 0) }}</span>
                    </div>
                    <div class="meta-item">
                ...
                <div class="order-actions">
                    @if($order->status == 'pending')
                        <button class="btn btn-success" onclick="window.location.href='/orders/{{ $order->id }}/payment'">
                            Complete Payment
                        </button>
                        <button class="btn btn-outline" onclick="window.location.href='/orders/{{ $order->id }}'">
                            View Details
                        </button>
                    @elseif($order->status == 'paid' || $order->status == 'accepted' || $order->status == 'in_progress')
                        <button class="btn btn-outline" onclick="window.location.href='/orders/{{ $order->id }}'">
                            View Details
                        </button>
                        @if($order->freelancer)
                            <button class="btn btn-outline" onclick="window.location.href='/chat/{{ $order->freelancer_id }}'">
                                Message Freelancer
                            </button>
                        @endif
                    @elseif($order->status == 'delivered')
                        <button class="btn btn-success" onclick="window.location.href='/orders/{{ $order->id }}/review'">
                            Accept & Review
                        </button>
                        <button class="btn btn-outline" onclick="window.location.href='/orders/{{ $order->id }}'">
                            View Details
        ...
        @empty
            <div class="empty-state">
                <h3>📦 No orders yet</h3>
                <p style="color: #666; margin-bottom: 20px;">Start by finding a freelancer for your project</p>
                <button class="btn btn-primary" onclick="window.location.href='/find-freelancers'">
                    Browse Freelancers
                </button>
            </div>
        @endforelse
